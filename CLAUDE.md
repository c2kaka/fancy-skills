...
| Path | Role |
|------|------|
| `skills/explain-clearly/` | Manual-only plain-language explanations and answer rewrites, with concrete examples and Mermaid diagrams when useful; Codex implicit invocation is disabled in `agents/openai.yaml`. |
...
| `skills/jira-auto-fix/` | Codex-native JIRA bug workflow with stable reproduction, evidence-backed diagnosis, a pyramid-structured review (`business reproduction chain → root cause → recommended solution`) written in plain language around one concrete example, a change-locality analysis (`references/change-locality.md`: variation → owner boundary → receivers → `已关住`/`已泄漏` verdict → locality budget), useful Mermaid visualization, approval gates, multimodal attachment inspection, regression testing, and local-commit-only delivery. Packet writing rules and a full worked example live in `references/review-packet-guide.md`. |
...
| `skills/review-coding-agent-design/` | Read-only, evidence-based review of coding-agent design documents and code, centered on complexity mechanisms, design-to-code traceability, boundary quality, error semantics, and grounded verification. |
...
5. To fix a JIRA bug, configure `skills/jira-auto-fix/.env` from `.env.example` and invoke **`jira-auto-fix`**; it must reproduce first, present a top-down review that explains the root cause and the fix in plain language through one concrete example, states whether the variation is contained at its owning boundary or leaked into many receivers, includes an evidence-backed Mermaid diagram when the flow is complex, then obtain solution approval before editing. The fix must stay within the locality budget declared in the packet; locality is not a license to refactor. It stops after a separately approved local commit and can hand the repair packet to **`jira-update`** in the same session.
...
14. To review coding-agent output as a design-and-implementation system, invoke **`review-coding-agent-design`** with the design document, requirements, code or diff, and available verification evidence. It traces claims into code, analyzes complexity and error semantics, and reports evidence-linked findings without editing or approving the target.
