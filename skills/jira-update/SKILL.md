...
- Never assume a status named `TEST`, a particular transition ID, or a tester username. Inspect the issue and its currently available transitions first. Require an exact user-confirmed target status name when later transitions are initially hidden.
- The reporter may be proposed as the tester, but never equate reporter with tester without user confirmation.
...
- Treat transition button names as case-sensitive project data. Examples and test fixtures in this skill are illustrative only and are never evidence for a live issue's workflow.
...
Choose a proposed transition from the returned IDs and propose a tester. If the desired return path is unavailable, explain the workflow constraint and stop; do not guess an intermediate transition.

When the only current transition is the project's start/progress action, record its exact button name, ID, and destination. Later transitions may be invisible until the preceding step completes. Use only project-specific workflow evidence confirmed by the user for hidden button names and the final testing status. Never copy names from this skill's examples or tests, and never normalize capitalization. The script resolves each confirmed button name only after it becomes visible and accepts it only when the match is exact and unique.

## Phase 3: Draft and confirm the exact operation

Read `references/comment-template.md` and draft a concise, factual comment from the handoff packet. The comment reports the code repair only; it must not claim that assignment or transition has already succeeded.
...
4. the resolved tester username and why it was proposed;
